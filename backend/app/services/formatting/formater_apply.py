...
class DocumentApplier:
    ...
    def apply(self) -> io.BytesIO:
        ...
        # 4) Tidy stray blank paragraphs so regenerated docs never show
        #    stacked empty lines / weird gaps.
        self._collapse_blank_paragraphs()
    ...
    def _collapse_blank_paragraphs(self) -> None:
        """Collapse runs of 2+ empty body paragraphs to a single spacer and drop
        trailing blanks. A paragraph that carries list numbering is never blank."""
        prev_blank = False
        paras = list(self.doc.paragraphs)
        for p in paras:
            has_num = p._p.find(".//" + qn("w:numPr")) is not None
            is_blank = not (p.text or "").strip() and not has_num
            if is_blank and prev_blank:
                ...
                continue
            prev_blank = is_blank
        # Drop a single trailing blank paragraph.
        tail = list(self.doc.paragraphs)
        if tail and not (tail[-1].text or "").strip() and tail[-1]._p.find(".//" + qn("w:numPr")) is None:
            try:
                tail[-1]._p.getparent().remove(tail[-1]._p)
            except Exception:
                pass
    ...
    def _add_list_item(self, element: ContentElement):
        ...
        # Empty list items render as just a stray bullet/number, which looks
        # broken — always drop the paragraph (and its numPr) when it has no
        # text, regardless of whether a native list style was applied.
        if not emitted_text:
            try:
        ...
        for row_idx, row_data in enumerate(element.rows):
            for col_idx, cell_data in enumerate(row_data.cells):
                ...
                for text_run in cell_data.content:
                    run = para.add_run(xml_safe(text_run.text))
                    self._apply_run_style(run, text_run.style_ref, text_run.inline_style)
    ...
    def _apply_para_style(
        ...
        # Tab stops (previously parsed but never applied).
        if ps.tab_stops:
            try:
                pf.tab_stops.clear_all()
                for pos in ps.tab_stops:
                    if pos is not None:
                        pf.tab_stops.add_tab_stop(Inches(float(pos)))
            except Exception:
                pass

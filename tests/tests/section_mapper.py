...
def map_sections(
    ...
    llm_result = _try_llm_mapping(slots, sections) if llm_available() else None
    mappings_by_slot: dict[str, Mapping] = {}
    if llm_result:
        for m in llm_result.mappings:
            mappings_by_slot[m.slot_id] = m

    # Fill gaps deterministically with a 1:1 greedy assignment so we never
    # bind two slots to the same draft section.
    used_section_indices: set[int] = {
        m.draft_section_idx
        for m in mappings_by_slot.values()
            ...
            for m in raw.get("mappings", []):
                ...
                mappings.append(
                    Mapping(
                        ...
                        draft_section_idx=m.get("draft_section_idx"),
